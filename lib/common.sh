...
# Is a module enabled as a Virtualmin plugin? Read from the config: the
# 'list-features' CLI lists only plugins that are domain features, so
# vmkit-check never appears there.
plugin_enabled_globally(){
  local cur; cur="$(get_kv /etc/webmin/virtual-server/config plugins)"
  case " $cur " in *" $1 "*) return 0 ;; *) return 1 ;; esac
}

# Enable a module as a Virtualmin plugin by adding it to the 'plugins=' line -
# what ticking the box on Features and Plugins does. We write the config
# directly, NOT 'virtualmin set-global-feature': that command validates the
# whole feature set on startup and fails during a fresh install, because virus
# scanning (clamd) is configured but not yet running (freshclam is still
# fetching the database and the post-install wizard has not run), so its virus
# check errors and it exits without touching our plugin. The config write has
# no such dependency and is what the panel does anyway. 'plugins=' is the ACTIVE
# list; 'plugins_inactive' is left alone so a newly enabled plugin is pre-ticked
# (default on) for new virtual servers.
plugins_add(){
  ...
  log "  enabled as a Virtualmin plugin: $mod"
}
