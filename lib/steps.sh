...
# Shared webmail (Roundcube), NOT tied to any domain - the Plesk model.
#
# One Roundcube under /var/www/vmkit-webmail, served on a localhost-only Apache
# backend (127.0.0.1:<port>). Every domain's webmail.<domain> is a ServerAlias
# on that domain's own vhost (so the domain's certificate covers the name) and
# is reverse-proxied to this backend by the directives step_webmail_template
# adds to the Apache website template. The address stays webmail.<domain> and
# no webmail virtual server is ever created. Roundcube is only an IMAP client:
# mail lives in Dovecot, users and passwords stay in Virtualmin, login is the
# full email address; the virtuser_file plugin resolves it to the sender
# identity via Postfix's virtual map.
step_webmail(){
  local dir=/var/www/vmkit-webmail port="${WEBMAIL_BACKEND_PORT:-8110}"
  local mycfg=/etc/webmin/mysql/config phpver
  phpver="$(ls /etc/php 2>/dev/null | sort -V | tail -1)"
  [ -n "$phpver" ] || { err "No PHP found under /etc/php; skipping webmail."; return 1; }

  # Extensions Roundcube needs (most already present with Virtualmin's LAMP).
  DEBIAN_FRONTEND=noninteractive apt-get install -y \
    "php${phpver}-mysql" "php${phpver}-intl" "php${phpver}-gd" \
    "php${phpver}-mbstring" "php${phpver}-xml" "php${phpver}-zip" "php${phpver}-curl" \
    >/dev/null 2>&1 || true

  local cfg="$dir/config/config.inc.php"
  if [ -f "$cfg" ]; then
    ok "Shared Roundcube already installed: $dir"
  else
    # Version: pinned in config.env, or the latest 1.6.x release.
    local ver="${ROUNDCUBE_VERSION:-}"
    if [ -z "$ver" ]; then
      ver="$(curl -fsSL https://api.github.com/repos/roundcube/roundcubemail/releases/latest 2>/dev/null |
             sed -n 's/.*"tag_name": *"\([^"]*\)".*/\1/p' | head -1)"
    fi
    [ -n "$ver" ] || { err "Could not determine a Roundcube version (set ROUNDCUBE_VERSION)."; return 1; }
    log "Installing shared Roundcube $ver into $dir  (download takes a while)"
    local tb="/tmp/roundcubemail-${ver}-complete.tar.gz"
    if ! curl -fsSL -o "$tb" \
         "https://github.com/roundcube/roundcubemail/releases/download/${ver}/roundcubemail-${ver}-complete.tar.gz"; then
      err "Could not download Roundcube $ver."; return 1
    fi
    rm -rf "/tmp/roundcubemail-${ver}"
    tar xzf "$tb" -C /tmp || { err "Could not extract Roundcube."; return 1; }
    mkdir -p "$dir"
    cp -a "/tmp/roundcubemail-${ver}/." "$dir/"
    # The web-reachable setup wizard is public_html/installer.php (Roundcube 1.6+
    # webroot is public_html). Remove it, and any legacy top-level installer dir.
    rm -f "$dir/public_html/installer.php"; rm -rf "$dir/installer"

    # Database. Root credentials come from Webmin's MySQL module config, because
    # Virtualmin sets a MySQL root password (socket auth as root is refused).
    local myuser mypass dbpass
    myuser="$(get_kv "$mycfg" login)"; [ -n "$myuser" ] || myuser=root
    mypass="$(get_kv "$mycfg" pass)"
    local mroot=(mysql -u"$myuser"); [ -n "$mypass" ] && mroot=(mysql -u"$myuser" -p"$mypass")
    dbpass="$(gen_pass)"
    if ! "${mroot[@]}" -e "CREATE DATABASE IF NOT EXISTS roundcube CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"; then
      err "Could not create the roundcube database."; return 1
    fi
    "${mroot[@]}" -e "CREATE USER IF NOT EXISTS 'roundcube'@'localhost' IDENTIFIED BY '${dbpass}';"
    "${mroot[@]}" -e "GRANT ALL ON roundcube.* TO 'roundcube'@'localhost'; FLUSH PRIVILEGES;"
    if ! "${mroot[@]}" roundcube < "$dir/SQL/mysql.initial.sql"; then
      err "Could not import the Roundcube schema."; return 1
    fi

    # config.inc.php. IMAP/SMTP go to localhost over TLS with name verification
    # off (the local certificate does not match 'localhost'); the hardening step
    # blocks cleartext auth, so plain localhost would be refused. des_key
    # encrypts the IMAP password in the session and Roundcube's default is
    # publicly known, so a random one is generated. proxy_whitelist trusts the
    # X-Forwarded-Proto from 127.0.0.1; trusted_hosts accepts only webmail.*.
    local deskey; deskey="$(LC_ALL=C tr -dc 'A-Za-z0-9' </dev/urandom 2>/dev/null | head -c 24 || true)"
    cat > "$cfg" <<PHP
<?php
\$config = [];
...
\$config['trusted_hosts'] = ['/^webmail\\..+\$/'];
\$config['plugins'] = ['virtuser_file'];
\$config['virtuser_file'] = '/etc/postfix/virtual';
\$config['enable_installer'] = false;
PHP
    ok "Shared Roundcube $ver installed."
  fi

  # Permissions (repeatable).
  chown -R root:www-data "$dir" 2>/dev/null || true
  chown -R www-data:www-data "$dir/temp" "$dir/logs" 2>/dev/null || true

  # A PHP-FPM pool of its own (repeatable): the shared tree is not a Virtualmin
  # domain, so it has no pool otherwise.
  local pool="/etc/php/${phpver}/fpm/pool.d/vmkit-webmail.conf"
  if [ ! -f "$pool" ]; then
    cat > "$pool" <<'POOL'
[vmkit-webmail]
user = www-data
group = www-data
listen = /run/php/vmkit-webmail.sock
...
POOL
    ...
    ok "PHP-FPM pool for the shared webmail created."
  fi

  # The localhost-only backend vhost. Roundcube 1.6+ serves from public_html/
  # (config/, logs/, temp/, vendor/ stay above it, out of the web root). Always
  # (re)written so a corrected DocumentRoot reaches an existing install too; a
  # reload picks that up, only a brand-new Listen needs a restart (newvh).
  local vh=/etc/apache2/sites-available/vmkit-webmail-backend.conf newvh=0
  [ -f "$vh" ] || newvh=1
  cat > "$vh" <<VHOST
Listen 127.0.0.1:${port}
<VirtualHost 127.0.0.1:${port}>
    ServerName vmkit-webmail.local
    ...
    <FilesMatch \.php\$>
        SetHandler "proxy:unix:/run/php/vmkit-webmail.sock|fcgi://localhost"
    </FilesMatch>
</VirtualHost>
VHOST
  a2ensite vmkit-webmail-backend >/dev/null 2>&1 || true
  if ! apache2ctl configtest >/dev/null 2>&1; then
    err "Apache config test failed after adding the webmail backend."; return 1
  fi
  # A newly added "Listen 127.0.0.1:<port>" is only bound by a full restart - a
  # graceful reload does not open a new socket. If the vhost was already there,
  # the socket is up and a reload is enough.
  if [ "$newvh" = 1 ]; then
  ...
  if curl -fsS -o /dev/null "http://127.0.0.1:${port}/" 2>/dev/null; then
    ok "Shared webmail backend answering on 127.0.0.1:${port}."
  else
    warn "Webmail backend did not answer on 127.0.0.1:${port} yet."
  fi
}

# Adds the shared-webmail routing to Virtualmin's templates, so every domain
# created afterwards automatically gets:
#   - Apache: ServerAlias webmail.<domain> + a Host-scoped reverse proxy to the
#     shared backend (acme-challenge left local so Let's Encrypt validates on :80)
#   - DNS: a webmail.<domain> A record in the new zone, so the domain's own
#     certificate covers the name (get_hostnames_for_ssl needs a zone record)
#     and it resolves - locally, and via the Cloudflare sync that models the
#     local zone. Added to Virtualmin's standard records, not replacing them.
#
# Written through Virtualmin's own template API (get_template/save_template) so
# the multi-line values are encoded the way Virtualmin expects - NOT by editing
# the config file by hand. Idempotent. Must run BEFORE the first domain so the
# main domain gets it too. Bails out (touching nothing) if the website template
# is not the expected populated default.
step_webmail_template(){
  need_virtualmin || return 1
  local port="${WEBMAIL_BACKEND_PORT:-8110}" out
  out="$(WEBMAIL_PORT="$port" virtualmin_perl vmkit-webmail-tmpl.pl '
    my $marker = "# vmin-kit: shared webmail";
    my $port = $ENV{"WEBMAIL_PORT"} || "8110";
    my $t = &get_template(0);
    my $web = $t->{"web"};
    # Guard: the default website template must be the real, populated block.
    # If it is empty we would otherwise replace it with just our directives and
    # break vhost generation (no DocumentRoot). Bail out instead of guessing.
    if (!defined($web) || index($web, "DocumentRoot") < 0) { print "notmpl\n"; exit 0; }
    my $changed = 0;
    # Apache: ServerAlias + Host-scoped proxy to the shared backend.
    # Virtualmin stores this template with TAB as the directive separator
    # (see the config file: one physical line, ^I between directives). Joining
    # with "\n" truncated the value, so the block never reached the template -
    # join with "\t".
    # ${IF-MAIL}...${ENDIF-MAIL}: only mail-enabled domains get webmail. Without
    # it the alias landed on the infra proxy sub-servers (webmin/usermin/docker)
    # too, putting useless webmail.<sub> names into their certs.
    if (index($web, $marker) < 0) {
      $t->{"web"} = $web.
        "\t\${IF-MAIL}".
        "\t$marker (webmail.<domain> -> 127.0.0.1:$port)".
    ...
    # DNS: add a webmail A record to the standard zone records ("none"/empty
    # means "use standard records"; appending keeps them and adds ours).
    my $dns = $t->{"dns"};
    $dns = "" if (!defined($dns) || $dns eq "none");
    if (index($dns, "webmail.\${DOM}") < 0) {
      $dns .= ($dns eq "" ? "" : "\n")."\${IF-MAIL}webmail.\${DOM}.\tA\t\${DNS_IP}\${ENDIF-MAIL}";
      $t->{"dns"} = $dns;
      $changed = 1;
      }
    if ($changed) { &save_template($t); print "changed\n"; }
    else          { print "present\n"; }
  ' 2>&1)"
  case "$out" in
    *changed*) ok "Webmail routing added to the Apache and DNS templates." ;;
    *present*) ok "Webmail routing already in the templates." ;;
    *notmpl*)  err "The Apache website template is not the expected default; left alone."; return 1 ;;
    *)         err "Could not update the templates: $out"; return 1 ;;
  esac
}
...
# self-register either - so plugins_add does it (writes the 'plugins=' line).
#
