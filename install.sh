...
# file by path, which would recreate it. The leading newline is for Ctrl+C at
# a prompt, where the cursor is still at the end of the question.
VMINKIT_STEPS_STARTED=0
trap '[ "$VMINKIT_STEPS_STARTED" = 1 ] || { printf "\n" >&3; log "No step ran; the log file was removed."; rm -f "$VMINKIT_LOGFILE"; }' EXIT
trap 'exit 130' INT
...
# The webmail routing goes into the Apache website template BEFORE any domain,
# so the main domain (and every later one) gets webmail.<domain> served from the
# one shared Roundcube. The shared backend itself is installed by step_webmail.
if is_truthy "$ROUNDCUBE"; then run_step step_webmail_template; fi
run_step step_dkim
