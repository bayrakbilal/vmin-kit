...
# --- Shared webmail (Roundcube): version / update ---------------------------
# Its own section, never part of the pass/fail total: an available update is
# not a broken dependency. When one is available the Update button runs it in a
# streamed progressive page (webmail_update_progressive.cgi) - human-triggered,
# never automatic. The latest-version lookup is best-effort with a short
# timeout, so a box with no outbound network just shows "could not check".
{
my $w = &webmail_status();
print &ui_subheading($text{'webmail_title'});
if (!$w->{'present'}) {
	print "<p>",$text{'webmail_none'},"</p>\n";
	}
elsif ($w->{'update'}) {
	print "<p>",&ui_text_color(&text('webmail_update', $w->{'installed'}, $w->{'latest'}), 'warn'),"</p>\n";
	# Human-triggered, streamed update - same shape as the other plugins'
	# progressive actions. No auto-update; this is the button that runs it.
	print &ui_form_start("webmail_update_progressive.cgi", "post"),
	      &ui_hidden("ver", $w->{'latest'}),
	      &ui_submit(&text('webmail_btn', $w->{'latest'})),
	      &ui_form_end();
	}
elsif ($w->{'latest'}) {
	print "<p>",&ui_text_color(&text('webmail_uptodate', $w->{'installed'}), 'success'),"</p>\n";
	}
else {
	print "<p>",&text('webmail_unknown', $w->{'installed'} || '?'),"</p>\n";
	}
}
