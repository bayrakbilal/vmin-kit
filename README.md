...
| `domain-defaults` | İlk domain oluşmadan önce: **spam ve virüs taraması kapalı** (`spam=0`, `virus=0`) ve rol adresleri `ROLE_ALIASES` ile sınırlanır (varsayılan `postmaster abuse`; hostmaster/webmaster çıkarılır). Posta adlandırmasına dokunulmaz — Virtualmin ne getiriyorsa o. |
