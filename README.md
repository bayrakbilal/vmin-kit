...
| `hardening` | Server-wide: Postfix drops VRFY and waits for TLS before AUTH, Dovecot forbids plaintext auth on non-TLS connections, BIND hides its version, Apache sends HSTS and nosniff on every vhost, and fail2ban gets longer escalating bans (values in `config.env`, written once) |
